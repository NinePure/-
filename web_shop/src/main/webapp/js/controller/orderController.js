//控制层
app.controller('orderController' ,function($scope,$controller,$location,OrderService,$window){
    ...
    $scope.shipping={};
    $scope.add_shipping=function(){
        OrderService.insertShippingToOrder($scope.shipping).success(
            function (response) {
                alert(response.message);
    ...
    // 显示状态
    $scope.status = ["等待买家付款","已付款","等待发货","已发货","退款中"];
    $scope.deleteStatus=["已下架","上架中"];
